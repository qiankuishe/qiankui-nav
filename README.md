...
- 📝 **笔记管理** - 支持置顶、Markdown 预览（编辑/预览切换）
- 📋 **剪贴板** - 保存文本、代码片段、图片，支持公开分享
- 🏷️ **公开便签** - 登录页展示公开便签，访客可查看
- 🔍 **站内搜索** - 快速搜索导航、笔记、剪贴板内容（300ms 防抖）
- 🌙 **深色模式** - 支持明暗主题切换
- 🎨 **主题配色** - 6 种配色方案（温暖棕、素雅白、海洋蓝、森林绿、玫瑰粉、紫罗兰）
- 📱 **响应式设计** - 适配桌面和移动端
- 🔐 **安全认证** - JWT 认证 + 登录限流保护
- 💾 **数据导入导出** - 支持 JSON 格式备份恢复
- 📥 **书签导入** - 支持从 Chrome/Edge/Firefox 导入书签
- 🕐 **最近访问** - 首页显示最近访问的链接（可在设置中开关）
- 📊 **访问统计** - 链接悬停显示访问次数和上次访问时间
- ⚡ **性能优化** - 图标缓存、模块懒加载、虚拟滚动
